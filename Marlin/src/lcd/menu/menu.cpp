...
// Last menu item - status screen shortcut button
menuPosition prev_history[6];
uint8_t prev_history_depth = 0;
screenFunc_t prev_currentScreen; // = status_screen;
...
void MarlinUI::_restore_history() {
...
void MarlinUI::lcd_menu_hop() {
  if (currentScreen != status_screen ) {
    _save_history();
    return_to_status();
  }
  else if (currentScreen == status_screen && prev_currentScreen != status_screen) {
    _restore_history();
    goto_screen(prev_currentScreen, prev_encoderPosition);
  }
}

void MarlinUI::goto_previous_menu_no_defer() {
